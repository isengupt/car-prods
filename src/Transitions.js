...
import { fragment } from "./fragmentAlt";
import disp1 from './img/disp1.jpg'
...
class Transition extends Component {
  ...
  componentDidMount() {
    ...
    this.clicker = document.getElementById("menu-wrapper");
  ...
  addObjects = () => {
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        displacement: {
          type: "f",
          value: new THREE.TextureLoader().load(disp1)
        },
...
/*   next = nextRoute => {
  ...
  next(){
    ...
    this.isRunning = true;
    let len = this.state.textures.length;
    let nextTexture =this.state.textures[(this.state.current +1)%len];
    this.material.uniforms.texture2.value = nextTexture;
    let tl = new TimelineMax();
    tl.to(this.material.uniforms.progress,this.duration,{
      value:1,
